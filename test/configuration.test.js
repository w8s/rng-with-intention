...
test('Configuration options', async (t) => {
  await t.test('deterministic mode produces same results', () => {
    const rngi = new RngWithIntention({
      includeTimestamp: false,
      includeEntropy: false
    });
    
    const result1 = rngi.draw('same intention', 1000);
    const result2 = rngi.draw('same intention', 1000);
    
    assert.equal(result1.index, result2.index, 
      'Same intention should produce same index in deterministic mode');
  });

  await t.test('different intentions produce different results in deterministic mode', () => {
    const rngi = new RngWithIntention({
      includeTimestamp: false,
      includeEntropy: false
    });
    
    const result1 = rngi.draw('intention A', 1000);
    const result2 = rngi.draw('intention B', 1000);
  ...
  await t.test('default mode includes both timestamp and entropy', () => {
    const rngi = new RngWithIntention();
    
    // Even same intention at "same" time should differ due to entropy
    const results = [];
    for (let i = 0; i < 5; i++) {
      results.push(rngi.draw('same intention', 1000));
    }
